...
import { Loader2, Mail, Lock, User, Phone, ArrowRight, ChevronDown, RefreshCw, Sparkles } from "lucide-react";
...
function InputRow({ icon: Icon, type, name, placeholder, value, onChange, required, minLength, autoFocus }: {
  icon: React.ElementType; type: string; name?: string; placeholder: string; value: string;
  onChange: (e: React.ChangeEvent<HTMLInputElement>) => void; required?: boolean; minLength?: number; autoFocus?: boolean;
}) {
  return (
    <div className="flex items-center gap-3 bg-white/[0.05] border border-white/[0.1] rounded-xl px-4 py-3.5 focus-within:border-indigo-500/60 focus-within:bg-white/[0.07] focus-within:shadow-[0_0_0_3px_rgba(99,102,241,0.12)] transition-all duration-200 group">
      ...
      <input type={type} name={name} required={required} minLength={minLength} value={value} onChange={onChange}
...
export default function Auth() {
  ...
  const reset = (newMode: Mode) => {
    setMode(newMode); setEmailStep("idle"); setPhoneStep("idle"); setError("");
  ...
  const handleEmailSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === "signin") { await login({ data: { email, password } }); }
      else { await register({ data: { name, email, password } as any }); }
    } catch (err: any) {
  ...
  const handleGoogleLogin = () => {
    if (!googleClientId || !isGoogleReady || !(window as any).google?.accounts?.oauth2) {
      setError("Google Sign-In is not configured. Add VITE_GOOGLE_CLIENT_ID to enable it."); return;
    }
    setGoogleLoading(true);
    const tokenClient = (window as any).google.accounts.oauth2.initTokenClient({
      client_id: googleClientId, scope: "openid email profile",
      callback: async (tokenResponse: { access_token?: string; error?: string }) => {
        if (!tokenResponse?.access_token || tokenResponse.error) {
          setGoogleLoading(false); setError("Google authentication failed. Please try again."); return;
        }
        try {
          const res = await fetch(`${API}/auth/google`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ accessToken: tokenResponse.access_token }) });
        ...
        } catch (e: any) { setError(e?.message || "Google login failed. Try email sign in."); }
  ...
  const handleSendOtp = async (e: React.FormEvent) => {
    ...
    setError(""); setOtpHint(null);
    try {
      const res = await fetch(`${API}/auth/phone/request-otp`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ phone }) });
      const data = await res.json();
  ...
  const handleOtpSubmit = async (e: React.FormEvent) => {
    e.preventDefault(); setError("");
    try {
      const res = await fetch(`${API}/auth/phone/verify-otp`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ phone, otp }) });
      const data = await res.json();
  ...
  const handleDemo = () => { enableDemo(); setLocation("/dashboard/jobs"); };
  ...
  return (
    <div className="min-h-screen bg-[#09090f] flex flex-col items-center justify-center p-4 relative overflow-hidden">
      ...
      <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="mb-8 relative z-10">
        <Logo size="md" />
      </motion.div>

      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45, ease: [0.25, 0.1, 0.25, 1] }} className="relative z-10 w-full max-w-[400px]">
        <div className="bg-white/[0.03] backdrop-blur-2xl border border-white/[0.08] rounded-2xl shadow-[0_32px_64px_rgba(0,0,0,0.5)] overflow-hidden">
          ...
          <div className="flex border-b border-white/[0.07]">
            {(["signin", "signup"] as Mode[]).map((m) => (
              <button key={m} onClick={() => reset(m)} className={`flex-1 py-4 text-sm font-semibold transition-all duration-200 relative ${mode === m ? "text-white" : "text-white/40 hover:text-white/70"}`}>
          ...
          <div className="p-7">
            ...
            <AnimatePresence>
              {error && (
                <motion.div key="error" initial={{ opacity: 0, height: 0, marginBottom: 0 }} animate={{ opacity: 1, height: "auto", marginBottom: 16 }} exit={{ opacity: 0, height: 0, marginBottom: 0 }}
            ...
            <div className="space-y-3">
              ...
              <AnimatePresence mode="wait">
                {phoneStep === "idle" && (
                  <motion.button key="phone-btn" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                ...
                {phoneStep === "enter_phone" && (
                  <motion.form key="phone-input" initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }} onSubmit={handleSendOtp} className="space-y-2.5">
                    <div className="flex items-center gap-2">
                      <button type="button" onClick={() => { setPhoneStep("idle"); setError(""); }} className="text-white/30 hover:text-white/60 transition-colors text-xs px-1 shrink-0">✕</button>
                ...
                {phoneStep === "enter_otp" && (
                  <motion.form key="otp-input" initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }} onSubmit={handleOtpSubmit} className="space-y-2.5">
                    ...
                    <div className="flex items-center gap-3 bg-white/[0.05] border border-white/[0.1] rounded-xl px-4 py-3.5 focus-within:border-indigo-500/60 focus-within:shadow-[0_0_0_3px_rgba(99,102,241,0.12)] transition-all duration-200">
                      <input type="text" maxLength={6} value={otp} onChange={e => setOtp(e.target.value)} autoFocus placeholder="6-digit OTP"
                    ...
                    <button type="button" onClick={() => { setPhoneStep("idle"); setError(""); }} className="w-full text-center text-xs text-white/30 hover:text-white/50 transition-colors py-1">
              ...
              {phoneStep === "idle" && (
                <AnimatePresence mode="wait">
                  {emailStep === "idle" ? (
                    <motion.button key="email-btn" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setEmailStep("form")}
                    ...
                    <motion.form key="email-form" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }} transition={{ duration: 0.2 }} onSubmit={handleEmailSubmit} className="space-y-2.5">
                      <div className="flex items-center gap-2 mb-1">
                        <button type="button" onClick={() => { setEmailStep("idle"); setError(""); }} className="text-white/30 hover:text-white/60 transition-colors text-xs px-1 shrink-0">✕</button>
                        <span className="text-xs text-white/40 font-medium">{mode === "signin" ? "Sign in with email" : "Sign up with email"}</span>
                      </div>
                      {mode === "signup" && <InputRow icon={User} type="text" name="name" placeholder="Full name" value={name} onChange={e => setName(e.target.value)} required autoFocus />}
                      <InputRow icon={Mail} type="email" name="email" placeholder="Email" value={email} onChange={e => setEmail(e.target.value)} required autoFocus={mode === "signin"} />
                      <InputRow icon={Lock} type="password" name="password" placeholder="Password" value={password} onChange={e => setPassword(e.target.value)} required minLength={6} />
                      <button type="submit" disabled={isLoading}
                        ...
                        {isLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : <>{mode === "signin" ? "Sign In" : "Create Account"}<ArrowRight className="w-4 h-4" /></>}
                      </button>
                      {mode === "signin" && (
                        <button type="button" onClick={async () => {
                          ...
                          if (res.ok) { setError("Reset link sent! Check your email inbox."); }
                          else { setError("Could not send password reset email."); }
            ...
            {phoneStep === "idle" && emailStep === "idle" && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }} className="mt-5 text-center">
                <button onClick={handleDemo} className="inline-flex items-center gap-1.5 text-sm text-white/30 hover:text-white/60 transition-colors group">
        ...
        <p className="text-center text-white/30 text-sm mt-5">
          {mode === "signin" ? (
            <>New here?{" "}<button onClick={() => reset("signup")} className="text-indigo-400/80 hover:text-indigo-400 font-medium transition-colors">Create a free account</button></>
          ) : (
            <>Already have an account?{" "}<button onClick={() => reset("signin")} className="text-indigo-400/80 hover:text-indigo-400 font-medium transition-colors">Sign in</button></>
          )}
        </p>
      </motion.div>
    </div>
